...
// Shared so surface-specific modules (e.g. meeting notes) can reuse the same
// Foundry client + credential instead of re-initializing them.
export { credential, projectClient };
...
agentApp.onConversationUpdate("membersAdded", async (context: TurnContext) => {
  await context.sendActivity(
    `Hi, I'm **Scout** \u2014 your Digital Scout assistant. Ask me about companies, needs, ` +
      `evaluations, and projects, or add me to a meeting and I'll capture the notes for you.`
  );
});
